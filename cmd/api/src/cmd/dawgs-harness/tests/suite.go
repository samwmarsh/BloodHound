...
func RunSuite(dbType DBType, db graph.Database) (TestSuite, error) {
	...
	//suite.NewTestCase("Fetch Nodes by Filter Item", FetchNodesByProperty(ad.ObjectID))
	suite.NewTestCase("Fetch Nodes by Indexed Item", FetchNodesByProperty(common.Name.String()))
